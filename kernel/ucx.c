...
{
	kcb_p->tcb_p->remaining_capacity = kcb_p->tcb_p->period == -1 ? 0 : kcb_p->tcb_p->remaining_capacity - 1;
	...
	do {
		...
		kcb_p->tcb_p->remaining_period = kcb_p->tcb_p->remaining_period < 1 ? kcb_p->tcb_p->period : kcb_p->tcb_p->remaining_period - 1;
	} while (kcb_p->tcb_p != tcb);
	printf("consegui chegar aqui\n");

	printf("numero de tarefas %d\n", number_of_tasks);
	printf("contador antes do incremento %d\n", task_counter);
	do {
		// do {
			kcb_p->tcb_p = kcb_p->tcb_p->tcb_next;
			printf("capacidade restante da tarefa com id %d: %d\n", kcb_p->tcb_p->id, kcb_p->tcb_p->remaining_capacity);
		// } while (kcb_p->tcb_p->remaining_capacity > 0);

		task_counter = kcb_p->tcb_p->remaining_capacity == 0 ? task_counter + 1 : task_counter;
	} while (kcb_p->tcb_p != tcb);

	printf("contador apos incremento %d\n", task_counter);
	
	if (task_counter == number_of_tasks) {
		printf("nenhuma tarefa possui capacidade restante! retornando...\n");
		return -1;
	}
